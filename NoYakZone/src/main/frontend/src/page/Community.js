...
const Community = () => {
  ...
  const filteredSlangData = slangData.filter(
    (slang) =>
      (searchTerm
        ? slang.word.toLowerCase().includes(searchTerm.toLowerCase())
        : true) && (initial ? getChosung(slang.word).startsWith(initial) : true)
  );

  const itemsPerPage = 9;
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredSlangData.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
          ...
          {Array.from(
            {
              length: Math.min(
                pagesPerGroup,
                totalPages - pageGroup * pagesPerGroup
              ),
            },
