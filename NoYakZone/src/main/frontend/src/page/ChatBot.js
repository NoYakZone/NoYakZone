...
const ChatBot = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("username") === null) {
      //비로그인
      setLoginUser(false);
      if (localStorage.getItem("chatcount") === null)
        localStorage.getItem("chatcount", 0); //무료 횟수
  ...
  const handleSendMessage = () => {
    ...
    if (loginUser) {
      ...
      axios
        .post("http://localhost:7890/chatbot/chat", chat) //채팅 입력 후 gpt 응답
        .then((response) => {
      ...
      if (currentCount > 4) {
        ...
        //무료 횟수가 소진되기 전 gpt 응답 가능
        const chat = [...messages, userMessage].map((msg) => ({
          //지금까지 대화내용 + 자신의 내용을 임시 배열로
          text: msg.text,
          bot: msg.bot,
        }));

        axios
          .post("http://localhost:7890/chatbot/getchat", chat) //채팅 입력 후 gpt 응답
          .then((response) => {
            const botMessage = {
              //배열에 먼저 봇 메세지 추가
              index: getLastMessageIndex(),
  ...
  const chatHistory = () => {
    ...
    axios
      .get(
        `http://localhost:7890/chatbot?id=${localStorage.getItem("username")}`
      )
      .then((response) => {
        if (response.data) {
      ...
      {isChatbotOpen && (
        ...
        >
          <div className="chatbot-header">
            <div className="chatbot-title">마약 상담 챗봇</div>
            ...
            {messages.map((message, index) => (
              <div
                ...
                className={`chatbot-message ${
                  message.bot ? "bot-message" : "user-message"
                }`}
              >
