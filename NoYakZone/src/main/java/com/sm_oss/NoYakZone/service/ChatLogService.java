...
public class ChatLogService {
    ...
    public List<ChatLog> getResponse(String id, String message) {// api호출 시 첫 실행
        ...
        List<ChatLog> currentLogs=chatLogRepository.findByUserId(id);
        int size = currentLogs.size();
        if (size <= 2) {
            return currentLogs;//최신 데이터 가져오기
        } else {
            return currentLogs.subList(size - 2, size);
        }
    }
